...
export class LanguageServerService {
  ...
  config = { domain: '', endpoint: { hlasm: 'asmServer', json: 'jsonServer' } };
  ...
  constructor(@Inject(Angular2InjectionTokens.LOGGER) private log: ZLUX.ComponentLogger) {
    // Auto-detect WebSocket domain from the Zowe Desktop host
    if (!this.config.domain) {
      const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
      this.config.domain = `${protocol}//${window.location.host}`;
    }
  }
